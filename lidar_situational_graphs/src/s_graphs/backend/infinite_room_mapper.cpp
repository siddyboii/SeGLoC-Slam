...
bool InfiniteRoomMapper::insert_infinite_room(
  ...
  if ((infinite_rooms.empty() || room_data_association == -1)) {
    ...
    assigned_room.id = det_infinite_room.id;

    // Fill cluster_center (we have Eigen::Isometry3d cluster_center argument)
    try {
      assigned_room.cluster_center.x = cluster_center.translation().x();
      assigned_room.cluster_center.y = cluster_center.translation().y();
      assigned_room.cluster_center.z = cluster_center.translation().z();
    } catch (...) {}

    // If you have the room_center available or can synthesize from 'room_center' arg, fill it:
    try {
      Eigen::Quaterniond q(room_center.linear());
    ...
    // Fill plane ids from room_plane1_pair and room_plane2_pair (plane_data_list)
    try {
      // situational_graphs_msgs::msg::PlaneData pd1;
      // pd1.id = room_plane1_pair.plane_id;
      // assigned_room.x_planes.push_back(pd1);
          assigned_room.x_planes.push_back(
            convertG2OPlaneToPlaneData(room_plane1_pair.plane_unflipped, room_plane1_pair.plane_id)
          );
      // situational_graphs_msgs::msg::PlaneData pd2;
      // pd2.id = room_plane2_pair.plane_id;
      // assigned_room.x_planes.push_back(pd2);  // if plane_type==X, else adjust accordingly
      assigned_room.y_planes.push_back(
            convertG2OPlaneToPlaneData(room_plane2_pair.plane_unflipped, room_plane2_pair.plane_id)
          );
    } catch (...) {}
    ...
    shared_graph_mutex.unlock();
    std::cout << "Else infinite found room with room id  " << room_id << std::endl;

  ////////////////////////////////////////////////////////////////////////////
  Eigen::Isometry3d matched_room_estimate;
    {
      shared_graph_mutex.lock();
      auto it_inf = infinite_rooms.find(room_data_association);
      if (it_inf != infinite_rooms.end() && it_inf->second.node) {
        matched_room_estimate = it_inf->second.node->estimate();
      } else {
        matched_room_estimate = room_center; // fallback
      }
      shared_graph_mutex.unlock();
    }
    ...
    // populate canonical room_center
    ...
    // populate cluster_center from cluster_center arg
    try {
      assigned_room.cluster_center.x = cluster_center.translation().x();
      assigned_room.cluster_center.y = cluster_center.translation().y();
      assigned_room.cluster_center.z = cluster_center.translation().z();
    ...
    // Fill plane ids from room_plane1_pair & room_plane2_pair (function args)
    try {
      // situational_graphs_msgs::msg::PlaneData pd1;
      // pd1.id = room_plane1_pair.plane_id;
      // assigned_room.x_planes.push_back(pd1);
      ...
      // situational_graphs_msgs::msg::PlaneData pd2;
      // pd2.id = room_plane2_pair.plane_id;
      // assigned_room.x_planes.push_back(pd2);
      assigned_room.y_planes.push_back(
            convertG2OPlaneToPlaneData(room_plane2_pair.plane_unflipped, room_plane2_pair.plane_id)
          );
    } catch (...) {}
