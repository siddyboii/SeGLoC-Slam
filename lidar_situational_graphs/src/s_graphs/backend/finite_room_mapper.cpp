...
FiniteRoomMapper::FiniteRoomMapper(const rclcpp::Node::SharedPtr node,
  ...
  node_obj = node;
  // QoS: transient_local so late subscribers get the last assignment
  rclcpp::QoS qos(rclcpp::KeepLast(10));
  qos.transient_local();

  room_data_assigned_pub_ = node_obj->create_publisher<situational_graphs_msgs::msg::RoomsData>(
      "room_segmentation/room_data_assigned", qos);
  room_information =
...
bool FiniteRoomMapper::factor_rooms(
  ...
  if ((rooms_vec.empty() || room_data_association == -1)) {
    ...
    std::cout << "If finite found room with room id  " << det_room.id << std::endl;
    room_id = det_room.id;
    ...
    ////////Adding my publisher here//////////////////////
    situational_graphs_msgs::msg::RoomsData assigned_msg;
    assigned_msg.header.stamp = node_obj->now();
    assigned_msg.header.frame_id = "";  // optional: set frame id if needed

    // Build RoomData from the det_room and function arguments
    situational_graphs_msgs::msg::RoomData assigned_room;
    assigned_room.id = det_room.id;

    // Fill room_center from Eigen::Isometry3d room_center (argument)
    {
      Eigen::Quaterniond q(room_center.linear());  // rotation matrix -> quaternion
      assigned_room.room_center.position.x = room_center.translation().x();
      assigned_room.room_center.position.y = room_center.translation().y();
      assigned_room.room_center.position.z = room_center.translation().z();
      assigned_room.room_center.orientation.x = q.x();
      assigned_room.room_center.orientation.y = q.y();
      assigned_room.room_center.orientation.z = q.z();
      assigned_room.room_center.orientation.w = q.w();
    }

    // If cluster_array is available (this function has cluster_array param), copy it
    try {
      assigned_room.cluster_array = cluster_array;
    } catch (...) {
      // ignore if any issue
    }

    // Populate x_planes and y_planes with plane ids from x_room_pair_vec and y_room_pair_vec
    try {
      // for (const auto &px : x_room_pair_vec) {
      //   situational_graphs_msgs::msg::PlaneData pd;
      //   pd.id = px.plane_id;
      //   pd.plane_points = px.plane_unflipped;
      //   assigned_room.x_planes.push_back(pd);
      // }
      for (const auto& px : x_room_pair_vec)
        {
          assigned_room.x_planes.push_back(
            convertG2OPlaneToPlaneData(px.plane_unflipped, px.plane_id)
          );
        }
    } catch (...) {}

    try {
      // for (const auto &py : y_room_pair_vec) {
      //   situational_graphs_msgs::msg::PlaneData pd;
      //   pd.id = py.plane_id;
      //   //pd.plane_points = px.plane_unflipped;
      //   assigned_room.y_planes.push_back(pd);
      // }
      for (const auto& py : y_room_pair_vec)
        {
          assigned_room.y_planes.push_back(
            convertG2OPlaneToPlaneData(py.plane_unflipped, py.plane_id)
          );
        }
    } catch (...) {}

    assigned_msg.rooms.push_back(assigned_room);

    // Publish
    room_data_assigned_pub_->publish(assigned_msg);
    ...
    shared_graph_mutex.unlock();
    std::cout << "Else finite found room with room id  " << room_id << std::endl;


    ////////////////////////////////////////////////////////////////
    Eigen::Isometry3d matched_room_estimate;
    {
      shared_graph_mutex.lock();
      auto it_room = rooms_vec.find(room_data_association);
      if (it_room != rooms_vec.end() && it_room->second.node) {
        matched_room_estimate = it_room->second.node->estimate();
      } else {
        // fallback to provided room_center if node not found (very unlikely)
        matched_room_estimate = room_center;
      }
      shared_graph_mutex.unlock();
    }

    // Build RoomsData message with one RoomData entry
    situational_graphs_msgs::msg::RoomsData assigned_msg;
    assigned_msg.header.stamp = node_obj->now();
    assigned_msg.header.frame_id = "";

    situational_graphs_msgs::msg::RoomData assigned_room;
    assigned_room.id = room_data_association;

    // Fill room_center from the matched room node estimate
    {
      Eigen::Quaterniond q(matched_room_estimate.linear());
      assigned_room.room_center.position.x = matched_room_estimate.translation().x();
      assigned_room.room_center.position.y = matched_room_estimate.translation().y();
      assigned_room.room_center.position.z = matched_room_estimate.translation().z();
      assigned_room.room_center.orientation.x = q.x();
      assigned_room.room_center.orientation.y = q.y();
      assigned_room.room_center.orientation.z = q.z();
      assigned_room.room_center.orientation.w = q.w();
    ...
    // Use provided room_center (candidate) as cluster_center for the published message
    try {
      assigned_room.cluster_center.x = room_center.translation().x();
      assigned_room.cluster_center.y = room_center.translation().y();
      assigned_room.cluster_center.z = room_center.translation().z();
    } catch (...) {
      // ignore if not available
    }

    // Populate x_planes and y_planes IDs from the available pair vectors
    try {
      // for (const auto &px : x_room_pair_vec) {
      //   situational_graphs_msgs::msg::PlaneData pd;
      //   pd.id = px.plane_id;
    ...
    // Copy cluster_array (function has cluster_array param)
    try {
      assigned_room.cluster_array = cluster_array;
    } catch (...) {}
