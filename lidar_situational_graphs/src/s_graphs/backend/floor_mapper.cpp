...
void FloorMapper::lookup_floors(
  ...
  // Safety check: ensure graph_slam is valid
  if (!graph_slam) {
...
int FloorMapper::associate_floors(const Eigen::Vector3d& floor_center,
  ...
  // Return early if floors_vec is empty to avoid potential issues
  if (floors_vec.empty()) {
  ...
  for (const auto& mapped_floor : floors_vec) {
    ...
    if (mapped_floor.second.node == nullptr) {
      continue;
    }
    shared_graph_mutex.lock();
